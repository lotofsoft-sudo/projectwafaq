...
import { Project, PurchaseOrder, User, Document } from '../types';
import PurchaseOrdersManager from './PurchaseOrdersManager';

interface ProjectPurchaseOrdersViewProps {
  projects: Project[];
...
export default function ProjectPurchaseOrdersView({
  projects,
  purchaseOrders,
  setPurchaseOrders,
  currentUser,
  ...
  documents,
  setDocuments,
}: ProjectPurchaseOrdersViewProps) {
  ...
  return (
    <div id="purchase-orders-view-root" className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-200 p-5 md:px-8 flex flex-col md:flex-row md:items-center md:justify-between shrink-0 gap-4">
        <div>
          <span className="text-xs font-mono font-bold text-indigo-600 uppercase tracking-widest block mb-1">Procurement & Materials</span>
          <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Contract Purchase Orders</h2>
          <p className="text-xs text-slate-500 mt-1">Formulate, dispatch, and monitor commercial purchase orders and supplier procurement pipelines.</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 w-full">
        ...
        <div 
          id="purchase-orders-project-sidebar" 
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50/60">
          ...
          <p className="text-[10px] text-slate-400 mt-1">Select a project to filter POs</p>
        ...
        <div className="flex-1 overflow-y-auto p-2.5 space-y-1 bg-slate-50/30">
          ...
          >
            ...
            <div>
              <h4 className="text-xs font-bold text-slate-800">All Client POs</h4>
              <p className="text-[10px] text-slate-400 mt-0.5">Show POs received for all projects & clients</p>
            </div>
          </button>
          ...
          {filteredProjects.length === 0 ? (
            ...
            filteredProjects.map(p => {
              ...
              const projectPOsCount = purchaseOrders.filter(po => po.projectId === p.id).length;
              return (
                <button
                  key={p.id}
                  onClick={() => {
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-mono font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">
                      {projectPOsCount} {projectPOsCount === 1 ? 'PO' : 'POs'}
                    </span>
                  </div>
      ...
      {/* 2. RIGHT PANEL: PO management viewport */}
      <div 
        id="purchase-orders-data-viewport" 
            ...
            <div className="overflow-hidden">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold text-slate-900 truncate">
                  {activeProject ? `${activeProject.name} - Purchase Orders` : 'Central Purchase Orders (PO) Directory'}
                </h3>
              </div>
              <p className="text-[10px] text-slate-400 truncate mt-0.5">
                {activeProject 
                  ? `Viewing registered purchase orders received from client ${activeProject.clientName} for this project.`
                  : 'Manage drafts, registered, and signed Client Purchase Orders globally across all projects'
                }
              </p>
            </div>
          </div>
